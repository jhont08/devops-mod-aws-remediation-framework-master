resource "aws_sns_topic" "remediator_notifications" {
...
data "aws_iam_policy_document" "remediator_policy_document" {
  ...
  statement {
    ...
    resources = [
      aws_sns_topic.remediator_notifications.arn
    ]
  }
  statement {
    actions = [
...
resource "aws_lambda_function" "remediator" {
  ...
  environment {
    variables = {
      ...
      CREATE_ISSUES                  = var.create_issues
      ...
      NOTIFICATION_TOPIC             = aws_sns_topic.remediator_notifications.arn
